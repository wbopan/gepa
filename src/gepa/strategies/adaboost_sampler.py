...
class PMaxBatchSampler(BatchSampler[DataId, DataInst]):
    ...
    - Never-solved samples (best_score == 0): AdaBoost weight update
      - May be difficult, boost weight to get more training opportunities

    This is the inverse of PAdaBoostBatchSampler:
    - PAdaBoost: solvable samples get AdaBoost updates, unsolvable samples frozen
    - PMax: solvable samples reset to 1.0, unsolvable samples get AdaBoost updates
    """
    ...
    def _update_weights_from_state(self, state: GEPAState) -> None:
        ...
        Uses PMax logic: once-solved samples (best_score > 0) have their weights
        reset to 1.0, while never-solved samples (best_score == 0) get AdaBoost
        weight updates.
        """
        pareto_front = state.pareto_front_valset
